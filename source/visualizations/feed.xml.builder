...
xml.feed "xmlns" => "http://www.w3.org/2005/Atom" do
  ...
  xml.link "href" => URI.join(site_url, blog('visualizations').options.prefix.to_s)
  xml.link "href" => URI.join(site_url, current_page.path), "rel" => "self"
  xml.updated(blog('visualizations').articles.first.date.to_time.iso8601) unless blog('visualizations').articles.empty?
